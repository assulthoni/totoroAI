  ...
  if (!handlersSet && bot) {
    ...
    bot.on("contact", async (ctx) => {
      try {
        ...
        logError("contact", e);
        await ctx.reply(
          "Failed to record your phone number. Please try again later.",
        );
      }
    });
    bot.on("text", async (ctx) => {
      ...
      try {
        ...
        if (action === "create_transaction" && result.data) {
          const { type, amount, category, description, expense_date } =
            result.data as {
        ...
        if (action === "read_transactions") {
          ...
          if (filters.end_date)
            q = q.lte("expense_date", new Date(filters.end_date).toISOString());
          ...
          if (aggregate.total_balance || aggregate.sum_by_type) {
            ...
            const parts: string[] = [];
            if (aggregate.sum_by_type)
              parts.push(
                `Income ${totals.income}, Expenses ${totals.expense}, Savings ${totals.savings}`,
              );
          ...
          const lines = data
            ...
            .map(
              (t) =>
                `${t.type} ${t.amount} ${t.category} on ${new Date(t.expense_date).toISOString().slice(0, 10)}`,
            );
          const suffix =
            data.length > 10 ? ` and ${data.length - 10} more` : "";
        ...
        if (
          ...
          if (match.date)
            q = q.eq("expense_date", new Date(match.date).toISOString());
          if (match.start_date)
            q = q.gte("expense_date", new Date(match.start_date).toISOString());
          if (match.end_date)
            q = q.lte("expense_date", new Date(match.end_date).toISOString());
        ...
        if (typeof result.isTransaction === "boolean") {
          ...
          await ctx.reply(
            result.reply ||
              "I'm not sure how to handle that. Can you rephrase?",
          );
          return;
        }
        await ctx.reply(
